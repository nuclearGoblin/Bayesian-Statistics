...
so $\phi|k,\lambda,y$ ~ Gamma$\left(1+\sum_{i=k+1}^{112}y_i,112-k+1/2\right)$
...
probsgen = function(lambda,phi){
  ...
  for(i in 1:nrow(y)){
    ...
    list = list-max(list) #values "divided" by the mean to prevent overflow
    #list = exp(list)     #(^ since we are inside exponent)
  }
...
for (i in 1:B){ #run a Gibbs sampler for B cycles
  ...
  probunexp = probsgen(lambda,phi); probunnorm = exp(probunexp); 
  probtotal = sum(probunnorm); problist = probunnorm/probtotal
  cmf = numeric(nrow(y))
  for(j in 1:nrow(y)){cmf[j] = sum(head(problist,j))} #find the cumulative density
  k = findInterval(kgen[i],cmf); kpost[i] = k
