...
generated quantities{
  ...
  for(i in 1:n){ll[i] = normal_lpdf(Wing[i] | mu[i],sqrt(sigs));}
}
"
...
where weight is given in gm.

Interpretation: Each gram of weight corresponds to, on average, a 0.193 mm increase in wingspan. There is an additional offset of 167 mm. For example, the wingspan of a "weightless" hawk (if hawks could be weightless) would be 167 mm, and the wingspan of a hawk weighing 1000 gm would be 360 mm.
...
```{r}
print("ANOVA")
WAIC(fit_anova$ll) #WAIC(ll) for n records, s samples
looic(mod_anova)#looic(mod) for a bayesian regression model; built on stan

print("Linear"); WAIC(fit_linear$ll); looic(mod_linear)
print("Parallel"); WAIC(fit_parallel$ll); looic(mod_parallel)
print("Separate"); WAIC(fit_separate$ll); looic(mod_separate)
```
